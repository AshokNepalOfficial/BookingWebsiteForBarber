<!-- NAVIGATION -->
<nav class="fixed w-full z-40 transition-all duration-300 py-2" id="navbar">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-3">
                @auth
                    ...
                    <a href="{{ route('receptionist.dashboard') }}" class="text-white hover:text-gold-500 text-xs font-medium tracking-widest transition-colors">
                        <i class="fas fa-user-circle mr-1"></i> DASHBOARD
                    </a>
                    @elseif(Auth::user()->role === 'barber')
                    <a href="{{ route('barber.dashboard') }}" class="text-white hover:text-gold-500 text-xs font-medium tracking-widest transition-colors">
                        <i class="fas fa-user-circle mr-1"></i> MY SCHEDULE
                    </a>
                    @else
                ...
                @endauth
                <button onclick="openBooking()" class="bg-gold-500 hover:bg-gold-400 text-dark-900 font-bold text-xs py-2 px-6 rounded-sm transition-all shadow-lg hover:shadow-gold-500/20">
                    BOOK NOW
                </button>
            </div>
            <button onclick="toggleMobileMenu()" class="md:hidden text-white text-2xl"><i class="fas fa-bars"></i></button>
        </div>
    </div>
    <!-- Mobile Menu -->
    <div id="mobile-menu" class="hidden bg-dark-900 border-t border-gray-800 md:hidden absolute w-full">
        <div class="flex flex-col p-4 space-y-4 text-center">
            ...
            @auth
                ...
                @elseif(Auth::user()->role === 'receptionist')
                <a href="{{ route('receptionist.dashboard') }}" class="text-white py-2 font-medium">
                    <i class="fas fa-user-circle mr-1"></i> DASHBOARD
                </a>
                @elseif(Auth::user()->role === 'barber')
                <a href="{{ route('barber.dashboard') }}" class="text-white py-2 font-medium">
                    <i class="fas fa-user-circle mr-1"></i> MY SCHEDULE
                </a>
                @else
                <a href="{{ route('profile.dashboard') }}" class="text-white py-2 font-medium">
                    <i class="fas fa-user-circle mr-1"></i> MY PROFILE
                </a>
                @endif
            @else
            ...
            @endauth
            <button onclick="openBooking()" class="bg-gold-500 text-black py-2 font-bold">BOOK NOW</button>
        </div>
    </div>
</nav>
