...
<div class="max-w-4xl">
    <form action="{{ route('admin.pages.store') }}" method="POST" class="bg-dark-900 rounded-lg border border-white/5 p-6">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="text" name="slug" value="{{ old('slug') }}"
                       class="input-dark" placeholder="e.g., privacy-policy (auto-generated if empty)">
                @error('slug')
                <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="content" rows="15" class="input-dark" placeholder="Enter page content (HTML allowed)">{{ old('content') }}</textarea>
            @error('content')
            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
