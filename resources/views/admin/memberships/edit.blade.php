...
@section('title', 'Edit Membership | Admin Panel')
...
<div class="max-w-3xl">
    <form action="{{ route('admin.memberships.update', $membership->id) }}" method="POST" class="bg-dark-900 rounded-lg border border-white/5 p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="membership_name" value="{{ old('membership_name', $membership->membership_name) }}" required
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="input-dark" 
                          placeholder="Describe the benefits of this membership...">{{ old('description', $membership->description) }}</textarea>
                @error('description')
                <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="duration_days" value="{{ old('duration_days', $membership->duration_days) }}" required
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="discount_percentage" value="{{ old('discount_percentage', $membership->discount_percentage) }}"
                ...
                <div>
                    ...
                    <input type="number" name="free_services_count" value="{{ old('free_services_count', $membership->free_services_count) }}" required
            ...
            <div class="space-y-3 pt-4 border-t border-white/10">
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="priority_booking" id="priority_booking" value="1" {{ old('priority_booking', $membership->priority_booking) ? 'checked' : '' }}
        ...
        <div class="flex gap-3 mt-8 pt-6 border-t border-white/10">
            <button type="submit" class="bg-gold-500 hover:bg-gold-600 text-dark-900 font-bold px-6 py-2 rounded transition-colors">
                <i class="fas fa-save mr-2"></i> Update Membership
            </button>
