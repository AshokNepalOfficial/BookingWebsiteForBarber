...
@section('page-title', 'Edit Blog Post: ' . $post->title)

@section('content')

<div class="max-w-4xl">
    <form action="{{ route('admin.blogs.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="bg-dark-900 rounded-lg border border-white/5 p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <div class="mb-2">
                <img src="{{ asset('storage/' . $post->featured_image) }}" class="w-48 h-32 object-cover rounded border border-white/10">
            </div>
            @endif
        ...
        <div class="mb-4">
            ...
            <textarea name="excerpt" rows="2" class="input-dark" placeholder="Brief summary for preview">{{ old('excerpt', $post->excerpt) }}</textarea>
            @error('excerpt')
            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="content" id="content-input" required class="hidden">{{ old('content', $post->content) }}</textarea>
            @error('content')
            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="meta_description" rows="2" class="input-dark" placeholder="Brief description for search engines">{{ old('meta_description', $post->meta_description) }}</textarea>
        ...
        <div class="flex gap-4 mb-6">
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $post->is_published) ? 'checked' : '' }}
                       class="w-4 h-4 text-gold-500 bg-dark-700 border-gray-600 rounded focus:ring-gold-500">
                <span class="text-sm text-gray-300">Publish post</span>
            </label>
        </div>
