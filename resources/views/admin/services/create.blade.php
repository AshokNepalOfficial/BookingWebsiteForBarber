...
@section('title', 'Create Service | Admin Panel')
...
<div class="max-w-3xl">
    <form action="{{ route('admin.services.store') }}" method="POST" class="bg-dark-900 rounded-lg border border-white/5 p-6">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="icon" value="{{ old('icon', 'fa-cut') }}"
                       class="input-dark" placeholder="fa-cut">
                <p class="text-xs text-gray-500 mt-1">Visit <a href="https://fontawesome.com/icons" target="_blank" class="text-gold-500 hover:underline">FontAwesome</a> for icon names</p>
                @error('icon')
                <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
