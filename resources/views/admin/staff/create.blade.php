...
@section('title', 'Create Staff Member | Admin Panel')
...
<div class="max-w-4xl">
    <form action="{{ route('admin.staff.store') }}" method="POST" enctype="multipart/form-data" class="bg-dark-900 rounded-lg border border-white/5 p-6">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="can_accept_bookings" value="1" {{ old('can_accept_bookings') ? 'checked' : '' }}
            ...
            <div>
                ...
                <textarea name="bio" rows="3" class="input-dark" placeholder="Brief description about the staff member">{{ old('bio') }}</textarea>
                @error('bio')
                <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex gap-3 mt-8 pt-6 border-t border-white/10">
            ...
            <a href="{{ route('admin.staff.index') }}" class="bg-dark-800 hover:bg-dark-700 text-gray-400 hover:text-white px-6 py-2 rounded transition-colors">
                Cancel
            </a>
        </div>
    </form>
