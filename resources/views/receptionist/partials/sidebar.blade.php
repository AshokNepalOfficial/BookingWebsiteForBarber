...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-dark-900 border-r border-white/10 flex flex-col h-full transition-transform duration-300 transform -translate-x-full lg:translate-x-0 lg:static lg:inset-auto">
    ...
    <div class="h-20 flex items-center justify-between px-6 border-b border-white/5 shrink-0">
        <div class="flex items-center">
            ...
            <img src="{{ asset('storage/' . $setting('logo_admin_dark')) }}" alt="Receptionist Logo" class="h-auto px-4 object-contain">
            @else
            <div class="w-8 h-8 bg-gold-500 rounded-full flex items-center justify-center text-dark-900 mr-3">
                <i class="fas fa-cut"></i>
            </div>
            <span class="font-serif text-xl font-bold">Receptionist</span>
            @endif
        </div>
    ...
    <nav class="flex-1 py-4 space-y-1 overflow-y-auto custom-scroll">
        ...
        <a href="{{ route('receptionist.dashboard') }}" class="nav-link {{ request()->routeIs('receptionist.dashboard') ? 'active' : '' }} flex items-center px-6 py-3 text-sm font-medium hover:bg-dark-800 hover:text-white text-gray-400">
            <i class="fas fa-chart-line w-6 text-center"></i> <span class="ml-2">Dashboard</span>
        </a>
        <a href="{{ route('receptionist.bookings.index') }}" class="nav-link {{ request()->routeIs('receptionist.bookings.*') ? 'active' : '' }} flex items-center px-6 py-3 text-sm font-medium hover:bg-dark-800 hover:text-white text-gray-400">
